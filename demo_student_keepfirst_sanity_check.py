...
def get_trajs(model, rgbs, S, iters=16, sw=None, counter=0, keypoint_type='harris', device='cpu', pips2start=False):
    ...
    if sw is not None and sw.save_this:
        ...
        # save the video
        out = cv2.VideoWriter('realtime_valid_sanitycheck_vis_' + str(counter).zfill(3) + '.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 4, (W, H))
...
if __name__ == '__main__':
    experiment = 'checkpoints/pipsUSv5_i6_S8_size256_256_kpsift_lr1e-4_A_Feb14_finetune_w_pipsv2_stride4'  
    main(experiment, device='cuda:0', model_name='model', stride=4)
