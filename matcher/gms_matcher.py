...
from matcher.graph_classic_matcher import HungarianMatcher, SinkhornMatcher
import numpy as np
import torch

DISABLE_GMS = True
THRESHOLD_FACTOR = 6  ## default: 6
...
class GmsMatcher:
    def __init__(self, matcher):
        ...
        # Grid initialize
        self.grid_size_left = Size(5, 5)  # default: (20, 20)  # save in tracking #### number of grids in the image
        # self.grid_size_left = Size(20, 20)  # default: (20, 20)  # save in tracking_V2
        # self.grid_size_left = Size(10, 10)  # save in tracking_V3
        ...
        if matcher == 'bfmatcher':
            ...
            hmatcher = HungarianMatcher()
            self.matcher = hmatcher
        elif matcher == 'sinkhorn':
            smatcher = SinkhornMatcher()
            self.matcher = smatcher
        else:
            raise ValueError('matcher must be one of bfmatcher, hungarian, sinkhorn')
    ...
    def compute_matches(self, img1, img2, keypoints_image1, descriptors_image1, keypoints_image2, descriptors_image2, backend='numpy'):
        ...
        self.image_size = img1.shape

        if self.gms_matches:
            self.empty_matches()

        # deal with the matcher
        if self.matcher_name == 'hungarian':
            all_matches = self.matcher.match(descriptors_image1, descriptors_image2, backend)
        elif self.matcher_name == 'sinkhorn':
            print("sinkhorn matcher only support pytorch, supress backend argument")
            if isinstance(descriptors_image1, np.ndarray):
                descriptors_image1 = torch.from_numpy(descriptors_image1).to(torch.float32)
            if isinstance(descriptors_image2, np.ndarray):
                descriptors_image2 = torch.from_numpy(descriptors_image2).to(torch.float32)
            all_matches = self.matcher.match(descriptors_image1, descriptors_image2, 'pytorch')
        else: # bfmatcher
    ...
    def get_inlier_mask(self, with_scale, with_rotation):
        ...
        # if disable gms:
        if DISABLE_GMS:
            print("Warning: gms is disable!")
            self.inlier_mask = [True for _ in range(self.matches_number)]
            return self.inlier_mask, self.matches_number
    ...
    def assign_match_pairs(self, grid_type):
        for i in range(self.matches_number):
            ...
            if lgidx < 0 or rgidx < 0:
                continue
            if lgidx >= self.grid_number_left or rgidx >= self.grid_number_right:
                continue
    ...
    def draw_matches(self, src1, src2, drawing_type, filename=None, visualize=False):
    ...
    def get_matches(self):
        matching_left = np.zeros((len(self.gms_matches), 2), dtype=np.int)
        matching_right = np.zeros((len(self.gms_matches), 2), dtype=np.int)

        for i in range(len(self.gms_matches)):
            left = self.keypoints_image1[self.gms_matches[i].queryIdx].pt
            left = tuple(map(int, left))
            right = tuple(sum(x) for x in zip(self.keypoints_image2[self.gms_matches[i].trainIdx].pt, (self.image_size[1], 0)))
            right = tuple(map(int, right))
            matching_left[i] = [left.y, left.x]
            matching_right[i] = [right.y, right.x]
        return matching_left, matching_right
