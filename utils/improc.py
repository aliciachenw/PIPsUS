...
class Summ_writer(object):
    def __init__(self, writer=None, global_step=1, log_freq=10, fps=8, scalar_freq=100, just_gif=False):
        self.writer = writer
    ...
    def summ_traj2ds_on_rgbs2(self, name, trajs, visibles, rgbs, valids=None, frame_ids=None, only_return=False, show_dots=True, cmap=None, linewidth=1):
        ...
        print("trajs", trajs.shape, 'visibles', visibles.shape, 'rgbs', rgbs.shape)
        B, S, C, H, W = rgbs.shape
        B, S2, N, D = trajs.shape
        assert(S==S2)
        ...
        for i in range(N):
            ...
            valid = valids[:,i] # S
            if valid[0]:
                rgbs_color = self.draw_circ_on_images_py(rgbs_color, traj, vis, S=S, show_dots=show_dots, cmap=cmap, linewidth=linewidth)
        return rgbs_color
        rgbs = []
        for rgb in rgbs_color:
            rgb = torch.from_numpy(rgb).permute(2, 0, 1).unsqueeze(0)
    ...
    def summ_epe2ds_on_rgbs(self, name, trajs_pred, trajs_gt, rgbs, valids=None, frame_ids=None, only_return=False, show_dots=False, cmap='coolwarm', vals=None, linewidth=1):
        """
        draw the end-point error (epe) on the rgbs
        """
        # trajs is B, S, N, 2
        # rgbs is B, S, C, H, W
        B, S, C, H, W = rgbs.shape
        B, S2, N, D = trajs_pred.shape
        assert(S==S2)
        B, S3, N1, D = trajs_gt.shape
        assert(S==S3)
        assert(N==N1)

        rgbs = rgbs[0] # S, C, H, W
        trajs_pred = trajs_pred[0] # S, N, 2
        trajs_gt = trajs_gt[0] # S, N, 2
        if valids is None:
            valids = torch.ones_like(trajs_pred[:,:,0]) # S, N
        else:
            valids = valids[0]
        ...
        for i in range(N):
            ...
            for t in range(S):
                rgbs_color[t] = self.draw_vec_on_image_py(rgbs_color[t], trajs_gt[t,i], trajs_pred[t,i], S=S, show_dots=show_dots, cmap=cmap_, linewidth=linewidth)

        for i in range(N): # plot the groundtruth point
            if cmap=='onediff' and i==0:
                cmap_ = 'spring'
            elif cmap=='onediff':
                cmap_ = 'winter'
    ...
    def draw_traj_on_image_py(self, rgb, traj, S=50, linewidth=1, show_dots=False, show_lines=True, cmap='coolwarm', val=None, maxdist=None):
        ...
        for s in range(S1):
            ...
            if show_lines and s<(S1-1):
                # have a sanity check for traj
                if 0 <= traj[s,0]<W and 0 <= traj[s,1]<H and 0 <= traj[s+1,0]<W and 0 <= traj[s+1,1]<H:
                    cv2.line(rgb,
                ...
                if s<(S1-1):
                    ...
                    if 0 <= traj[s,0]<W and 0 <= traj[s,1]<H and 0 <= traj[s+1,0]<W and 0 <= traj[s+1,1]<H:
                        cv2.line(rgbs[s],
                                ...
                                cv2.LINE_AA)
                if show_dots:
                    cv2.circle(rgbs[s], (int(traj[s,0]), int(traj[s,1])), linewidth, np.array(color_map(1)[:3])*255, -1)

        return rgbs



    def draw_vec_on_image_py(self, rgb, pt_gt, pt_pred, S=50, linewidth=1, show_dots=False, show_lines=True, cmap='coolwarm', val=None, maxdist=None):
        # all inputs are numpy tensors
        ...
        if val is not None:
            ...
            color = np.array(color_map(1)[:3]) * 255 # rgb
        # have a sanity check for traj
        if 0 <= pt_pred[0]<W and 0 <= pt_pred[1]<H and 0 <= pt_gt[0]<W and 0 <= pt_pred[1]<H:
            cv2.line(rgb,
                    (int(pt_gt[0]), int(pt_gt[1])),
                    (int(pt_pred[0]), int(pt_pred[1])),
                    (0,255,0),
                    linewidth,
                    cv2.LINE_AA)
    ...
    def draw_circ_on_images_py(self, rgbs, traj, vis, S=50, linewidth=1, show_dots=False, cmap=None, maxdist=None):
        ...
        for s in range(S1):
            ...
            if 0<=traj[s,0]<W and 0<=traj[s,1]<H:
