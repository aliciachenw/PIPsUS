...
def soft_argmax_heatmap_batched(softmax_val, threshold=5):
    ...
    return weighted_sum / sum_of_weights # b, n, t, 2


def heatmaps_to_points(
    all_pairs_softmax, # B, N, T, H, W
    threshold=5):
    """Convert heatmaps to points using soft argmax."""
    all_pairs_softmax = all_pairs_softmax.unsqueeze(2)
    out_points = soft_argmax_heatmap_batched(all_pairs_softmax, threshold)  # soft argmax in patch -> actually flow in patch coordinates, (ij) direction?
    # print("out_points.shape", out_points.shape) # B, N, T, 2
    # Note: out_points is now [x, y]; we need to divide by [width, height].
    # image_shape[3] is width and image_shape[2] is height.
    out_points = out_points.squeeze(2)
