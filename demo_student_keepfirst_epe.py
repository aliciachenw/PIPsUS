...
def get_trajs(teacher_model, student_model, rgbs, S, iters=16, sw=None, counter=0, keypoint_type='harris', device='cpu', pips2start=False):
    ...
    with torch.no_grad():
        for i in range(1, video_length):            
            ...
            preds_coords, _, _ = student_model(traj_pre, prev, curr, iters=1, beautify=True)
    ...
    if sw is not None and sw.save_this:
        ...
        # save the video
        out = cv2.VideoWriter('realtime_valid_epe_vis_' + str(counter).zfill(3) + '.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 4, (W, H))
...
if __name__ == '__main__':
    experiment = 'checkpoints/pipsUSv5_i6_S8_size256_256_kpsift_lr1e-4_A_Feb21_finetune_w_pipsv2+rand'  
