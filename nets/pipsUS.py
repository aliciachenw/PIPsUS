"""
more like pips2, only use first, -2, -4 frame, with previous flow 
"""
...
class PipsUS(nn.Module):
    ...
    def forward(self, trajs_previous, image_previous, image_curr, iters=3, feat_pre=None, valids=None, sw=None, beautify=False, return_feat=False):
        # trajs_previous: B x S x N x 2
        # image_previous: B x S x C x H x W
        # image_curr: B x 1 x C x H x W

        trajs_previous = trajs_previous / float(self.stride)
        coords = trajs_previous[:,-1,:,:].clone() # assume no motion, B x N x 2, also downsampled to feature map resolution
        trajs_history = trajs_previous[:,-self.seq_len:,:,:].clone()
        # NOTE: hacking to reuse previous keepfirst training code
        ...
        for itr in range(iters):
            ...
            flows_ = coords.unsqueeze(1).repeat(1,S,1,1) # B x N x 2 -> B x 1 x N x 2 -> B x S x N x 2
            flows_ = flows_ - trajs_history
        ...
        if return_feat:
            new_feat = utils.samp.bilinear_sample2d(fmaps_curr, coords[:,:,0], coords[:,:,1]).permute(0, 2, 1).unsqueeze(1) # B,1,N,C
            return coord_predictions1, new_feat
